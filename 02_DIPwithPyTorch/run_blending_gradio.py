...
def create_mask_from_points(points, img_h, img_w):
    ...
    # Reshape points to the required format for fillPoly
    # The fillPoly function requires points to be in the shape (num_polygons, num_points, 2)
    points = np.expand_dims(points, axis=0)  # Shape: (1, n, 2)
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    Args:
        foreground_img (torch.Tensor): Foreground image tensor of shape [batch_size, Channels, H, W].
        foreground_mask (torch.Tensor): Foreground mask tensor of shape [batch_size, 1, H, W].
        blended_img (torch.Tensor): Blended image tensor of shape [batch_size, Channels, H, W].
        background_mask (torch.Tensor): Background mask tensor of shape [batch_size, 1, H, W].
    ...
    # gradient_kernel_x = torch.tensor([[0, 0, 0], [1.0, -2, 1.0], [0, 0, 0]], device=foreground_img.device).view(1, 1, 3, 3)
    # gradient_kernel_y = torch.tensor([[0, 1.0, 0], [0, -2, 0], [0, 1, 0]], device=foreground_img.device).view(1, 1, 3, 3)
    gradient_kernel = torch.tensor([[0, 1.0, 0], [1, -4, 1], [0, 1.0, 0]]).view(1, 1, 3, 3).to(foreground_img.device)
    gradient_kernel = gradient_kernel.expand(foreground_img.size(1), -1, -1, -1)
    # # Expand kernels to match the number of channels
    # gradient_kernel_x = gradient_kernel_x.expand(foreground_img.size(1), -1, -1, -1)  # Shape: [Channels, 1, 3, 3]
    # gradient_kernel_y = gradient_kernel_y.expand(foreground_img.size(1), -1, -1, -1)  # Shape: [Channels, 1, 3, 3]
    fore_conv = torch.nn.functional.conv2d(foreground_img, gradient_kernel, padding=1, groups=foreground_img.size(1))
    blend_conv = torch.nn.functional.conv2d(blended_img, gradient_kernel, padding=1, groups=blended_img.size(1))
    # # Compute gradients using convolution
    # fore_conv_x = torch.nn.functional.conv2d(foreground_img, gradient_kernel_x, padding=1, groups=foreground_img.size(1))
    # fore_conv_y = torch.nn.functional.conv2d(foreground_img, gradient_kernel_y, padding=1, groups=foreground_img.size(1))
    # blend_conv_x = torch.nn.functional.conv2d(blended_img, gradient_kernel_x, padding=1, groups=blended_img.size(1))
    # blend_conv_y = torch.nn.functional.conv2d(blended_img, gradient_kernel_y, padding=1, groups=blended_img.size(1))
    # # 卷积后的大小与原图像相同
    # # Use masks to select the relevant pixels
    # # For foreground loss
    fore_mask = foreground_mask.expand_as(fore_conv)  # Expand to [batch_size, 1, H, W]
    back_mask = background_mask.expand_as(blend_conv)  # Expand to [batch_size, 1, H, W]
    loss = torch.sum((fore_conv[fore_mask>0] - blend_conv[back_mask>0])**2)/fore_mask.sum()
    # valid_fore_conv_x = fore_conv_x[fore_mask > 0]
    # valid_blend_conv_x = blend_conv_x[back_mask > 0]
    # loss_x = torch.sum((valid_fore_conv_x - valid_blend_conv_x) ** 2)
    # # For background loss
    # valid_fore_conv_y = fore_conv_y[fore_mask > 0]
    # valid_blend_conv_y = blend_conv_y[back_mask > 0]
    
    # loss_y = torch.sum((valid_fore_conv_y - valid_blend_conv_y) ** 2)
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    ...
    Args:
        foreground_image_original (PIL.Image): The original foreground image. [H,W,Channels]
        background_image_original (PIL.Image): The original background image.
        dx (int): Horizontal offset.
        dy (int): Vertical offset.
        polygon_state (dict): The current state of the polygon.
    ...
    # 将bg_mask_tensor转换为bool类型，True代表多边形内部，再使用expand函数扩展维数，-1代表不变,[1,3,H,W]
    ...
    for step in range(iter_num):
        ...
        # 第一个 blended_img是不看作变量的，只是单纯的数据，第二个blended_img是看作变量的，进行梯度回传进行优化。
        # Only blending in the mask region
    ...
    # .clamp()用于将结果限定在[0,1],permute将维度从[batch_size,Channels,H,W]变回[batch_size,H,W,Channels]
    # .squeeze()用于去除维度为1的维度,这里用于去除battch_size
